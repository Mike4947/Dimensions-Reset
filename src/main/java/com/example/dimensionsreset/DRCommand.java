...
public class DRCommand implements CommandExecutor, TabCompleter, PortalManager {
    ...
    // --- Methods from the PortalManager "Contract" ---
    // These are now guaranteed to exist, fixing the errors in PlayerListener.
    @Override
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, @NotNull String[] args) {
        if (args.length == 0) {
            ...
            sender.sendMessage(ChatColor.GRAY + "Use /dr gui for an easy-to-use menu.");
            return true;
        }
    ...
    public @Nullable List<String> onTabComplete(@NotNull CommandSender sender, @NotNull Command command, @NotNull String alias, @NotNull String[] args) {
        ...
        List<String> allCommands = new ArrayList<>(List.of("confirm", "reload"));
        if(sender.hasPermission("dimensionsreset.admin")) {
            allCommands.addAll(Arrays.asList("reset", "cancel", "status", "gui"));
        }
